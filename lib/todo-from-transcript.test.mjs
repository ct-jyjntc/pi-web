...
test("only the latest user turn contributes todos", async () => {
  const { deriveTodosFromTranscript } = await load();
  const messages = [
    { role: "user", content: [{ type: "text", text: "first" }] },
    assistantTodoCalls("old"),
    toolResult("Created #1: old task (pending)", "old"),
    { role: "user", content: [{ type: "text", text: "second" }] },
    assistantTodoCalls("new"),
    toolResult("Created #1: current turn only (in_progress)", "new"),
  ];
  const items = deriveTodosFromTranscript(messages);
  assert.equal(items.length, 1);
  assert.equal(items[0].subject, "current turn only");
  assert.equal(items[0].status, "in_progress");
});
