  ...
  return {
    ...
    factory(pi: ExtensionAPI) {
      ...
      pi.registerTool({
        ...
        label: "MCP",
        description:
          "Call tools on configured MCP servers (stdio or HTTP/SSE). " +
          "Use search/describe first, then tool+args. For HTTP servers that need login: " +
          "action=auth-start then auth-complete with the redirect URL. " +
          "Pass server when two servers expose the same tool name.",
        promptSnippet: "mcp: Call a configured MCP server tool",
        ...
        async execute(_id, raw, _signal, _onUpdate, ctx) {
          ...
          if (params.action === "list") {
            ...
            if (tools.length === 0) return textResult("No MCP tools connected.");
          ...
          if (params.search) {
            ...
            if (tools.length === 0) return textResult(`No MCP tools matched "${params.search}".`);
            return textResult(tools.map((tool) => `- ${tool.server}/${tool.name}: ${tool.description}`).join("\n"));
          }
          if (params.describe) {
