...
describe("user-scope budget is separate from project", () => {
  it("overflowing user scope leaves project scope writable", () => {
    ...
    retainMemoryFact(cwd, "user reviews PRs on tuesdays and thursdays", { settings });
    assert.throws(
      () => retainMemoryFact(cwd, "yet another long user preference " + "z".repeat(300), { settings }),
      /project memory at \d+\/500 chars.*would exceed the limit/,
    );
...
describe("buildMemoryInjectBlock", () => {
  it("injects project memory only when autoInject is enabled", () => {
    ...
    assert.doesNotMatch(block, /- user prefers dark theme/);
